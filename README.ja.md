...
- **コマンドラインからの切り替え**: スクリプトから `ccswitcher use Work` の一行で切り替えられます。App のみの環境でも `open "ccswitcher://use?account=Work"` が使えます。切り替えは App 内で実行されるため、メニュー操作と同じバックアップ・検証・キーチェーンの信頼関係をそのまま引き継ぎます。詳細は [docs/cli.md](docs/cli.md) を参照してください。
- **クォータの事前ウォームアップ**: 5 時間ウィンドウは時計ではなくアカウントの最初のリクエストで作成されます。9 時に始めるとウィンドウの終盤が昼休みで浪費され、13 時に戻っても待つことになります。8 時に極小のリクエストを 1 回送るよう設定すれば、境界がちょうど昼休みに重なります。各アカウントは自身のトークンでその場でウォームアップされ、切り替えは発生しないため、実行中の `claude` セッションに影響しません。詳細は [docs/prewarm.md](docs/prewarm.md) を参照してください。
