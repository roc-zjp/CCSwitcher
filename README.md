...
- **Command-Line Switching**: `ccswitcher use Work` from any script — or `open "ccswitcher://use?account=Work"` with nothing installed but the app itself. The switch runs inside the app, so it inherits the same backup, verification and keychain trust as a menu click. See [docs/cli.md](docs/cli.md).
- **Quota Pre-warming**: The 5-hour window is created by an account's first request, not by the clock — so starting at 9am leaves you waiting at 1pm for a window whose tail was spent at lunch. Schedule one tiny request for 8am and the boundary lands on your lunch break instead. Every account is warmed in place, using its own token, with no switching — an in-flight `claude` session is untouched. See [docs/prewarm.md](docs/prewarm.md).
