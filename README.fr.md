...
- **Changement en ligne de commande** : `ccswitcher use Work` depuis n'importe quel script — ou `open "ccswitcher://use?account=Work"` si seule l'application est installée. Le changement s'exécute dans l'application et hérite donc des mêmes sauvegarde, vérification et confiance du trousseau qu'un clic dans le menu. Voir [docs/cli.md](docs/cli.md).
- **Préchauffage du quota** : la fenêtre de 5 heures est créée par la première requête d'un compte, pas par l'horloge — commencer à 9 h fait consumer la fin de la fenêtre pendant le déjeuner, et vous attendez à 13 h. Une requête minuscule programmée à 8 h place la limite sur votre pause déjeuner. Chaque compte est préchauffé sur place avec son propre jeton, sans changement — une session `claude` en cours n'est pas affectée. Voir [docs/prewarm.md](docs/prewarm.md).
