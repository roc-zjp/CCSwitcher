...
- **Wechsel per Kommandozeile**: `ccswitcher use Work` aus jedem Skript — oder `open "ccswitcher://use?account=Work"`, wenn nur die App installiert ist. Der Wechsel läuft in der App und erbt damit dieselbe Sicherung, Überprüfung und Keychain-Vertrauensstellung wie ein Klick im Menü. Siehe [docs/cli.md](docs/cli.md).
- **Kontingent-Vorwärmen**: Das 5-Stunden-Fenster entsteht durch die erste Anfrage eines Kontos, nicht durch die Uhrzeit — wer um 9 Uhr beginnt, verliert das Ende des Fensters in der Mittagspause und wartet um 13 Uhr. Eine geplante winzige Anfrage um 8 Uhr legt die Grenze stattdessen auf die Mittagspause. Jedes Konto wird mit seinem eigenen Token an Ort und Stelle vorgewärmt, ohne Wechsel — eine laufende `claude`-Sitzung bleibt unberührt. Siehe [docs/prewarm.md](docs/prewarm.md).
