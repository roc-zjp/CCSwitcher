...
- **额度预热**：5 小时窗口由账号的第一次请求创建，与时钟无关——9 点上班才发第一条，窗口尾巴就耗在午休里，13 点回来还得干等。定时在 8 点发一条极小的请求，边界就落到午休上。每个账号都用自己的 token 原地预热，不切换账号，正在跑的 `claude` 会话不受影响。详见 [docs/prewarm.md](docs/prewarm.md)。
